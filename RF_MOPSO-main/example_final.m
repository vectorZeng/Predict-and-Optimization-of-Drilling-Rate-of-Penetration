...
% 获取最终 Pareto 前沿的适应度值和粒子位置
fit_values = abs(REP.pos_fit);
positions = abs(REP.pos);

% 可视化 Pareto 前沿
figure;
scatter(fit_values(:,1), fit_values(:,2), 'filled');
ylabel('TOB');
xlabel('ROP');
title('钻速优化');

% 如果有真实 Pareto 前沿，也可进行比较
if ~isempty(MultiObj.truePF)
    hold on;
    scatter(MultiObj.truePF(:,1), MultiObj.truePF(:,2), 'r', 'filled');
    legend('优化后', '优化前');
end
